...
def get_data_str(string):
    # (str) -> str
    """Gets all characters from input string that are in between curly braces."""

    left = string.find("{") + 1
    right = string.find("}", left)
    text = string[left:right].replace("$", "''").lstrip().rstrip("\n")
...
def getEq(line):  # Gets all data within constraints,substitutions
    ...
    for ch in line:  # read each character
        ...
        if ch == "{" and per == 0:
        ...
        elif ch == "$" and per == 0:  # either begin or end equation
            ...
            stringWrite += ch  # write the character

    return stringWrite.strip()
...
def modLabel(label):
    ...
    for i in range(0, len(label)):
        if isNumer and not label[i][0].isdigit():
            if len(num) > 1:
                newlabel += num
                isNumer = False
                num = ""
...
def main():
    ...
    with open("OrthogonalPolynomials.mmd", "r") as main:
        mainText = main.read()
    ...
    for i in range(0, len(lines)):
        ...
        if "\\begin{document}" in line:
            ...
            chapter = get_data_str(line)
            mainPrepend += (
            "\n== Sections in " + chapter + " ==\n\n<div style=\"-moz-column-count:2; column-count:2;-webkit-column-count:2\">\n")
        elif "\\part" in line:
            if get_data_str(line) == "BOF":
                parse = False
            elif get_data_str(line) == "EOF":
                parse = True
            elif parse:
                mainPrepend += ("\n<br />\n= " + get_data_str(line) + " =\n")
                head = True
        elif "\\section" in line:
            mainPrepend += ("* [[" + secLabel(get_data_str(line)) + "|" + get_data_str(line) + "]]\n")
            sections.append([get_data_str(line)])
    ...
    for i in range(0, len(lines)):
        ...
        # wiki.write("<div align=\"right\">Substitution(s): "+getEq(line)+"</div><br />\n")
            ...
            if "\\drmfname" in line and parse:
                wiki.write("<div align=\"right\">This formula has the name: " + get_data_str(line) + "</div><br />\n")
        elif math and parse:
            flagM = True
            eqs[len(eqs) - 1] += line
    ...
    for i in range(0, len(lines)):
        ...
        if "\\begin{equation}" in line:
            ...
            math = False
            comToWrite = "\n== Name ==\n\n<div align=\"left\">" + get_data_str(line) + "</div><br />\n" + comToWrite

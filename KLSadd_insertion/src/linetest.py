...
with open("KLSadd.tex", "r") as file:
    ...
    for word in entireFile:
        ...
        # 1/27/16 RS added \large\bf KLSadd: to make KLSadd additions appear
        # like the other paragraphs
        if("paragraph{" in word):
            temp = word[0:word.find(
                "{") + 1] + "\large\\bf KLSadd: " + word[word.find("{") + 1: word.find("}") + 1]
            entireFile[
                entireFile.index(word): entireFile.index(word) +
                1] = temp
    ...
    with open("newtempchap09.tex", "w") as temp9:
        ...
        # 1/29/16 add hyperref, xparse, cite packages after the footmisc
        # package
        entire9[
            footmiscIndex] += "\\usepackage[pdftex]{hyperref} \n\\usepackage {xparse} \n\\usepackage{cite} \n"
    ...
    with open("newtempchap14.tex", "w") as temp14:
        entire14[
            footmiscIndex14] += "\\usepackage[pdftex]{hyperref} \n\\usepackage {xparse} \n\\usepackage{cite} \n"

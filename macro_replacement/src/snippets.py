...
# regex for matching valid characters within another element
valid = r'(?:[^=s,;|])'
# regex for matching opening parentheses
open_paren = r'(?:\\left|\\Bigg|\\bigg|\\big|\\Big|\\bigl)'
# regex for matching closing parentheses
close_paren = r'(?:\\right|\\Bigg|\\bigg|\\big|\\Big|\\bigr)'
...
def subscript(num_args=1):
    ...
    pattern += _create_named_groups(
        _create_group_names(
            _frequencies["subscript"],
            "sub",
            end=num_args))
...
def superscript(num_args=1):
    ...
    pattern += r'(?:' + _create_named_groups(
        _create_group_names(
            _frequencies["superscript"],
            "sup",
            end=num_args)) + r'|' + simple_arg_list(num_args) + r')'
...
# parameters ending in _ex signify exclusivity (i.e. semi_first_ex means a
# ; ONLY comes after the first argument)


def semi_bar_arg_list(
        num_args,
        all_paren=False,
        semi_first_ex=False,
        semi_first=False,
    ...
    if any([semi_first, semi_last, bar_first, bar_last]):
        ...
        if bar_first:  # bar is after first argument ONLY
            ...
            pattern += _create_named_groups(
                _create_group_names(
                    _frequencies["semi_bar_arg_list"],
                    "arg",
                    end=1)) + r'\|' + _whitespace + r'*'
            start_loc += 1
    ...
    if not (semi_first_ex or semi_last_ex):
        ...
        # ensure that the number of groups, size of each one, and the number of
        # arguments all make sense
        if num_groups * group_size != expected:
            raise ValueError(
                "{0} * {1} != {2}".format(num_groups, group_size, expected))
    ...
    if semi_last or bar_last:
        if semi_last:  # semicolon is before last argument
            ...
            pattern += _whitespace + r'*;' + _create_named_groups(
                _create_group_names(
                    _frequencies["semi_bar_arg_list"],
                    "arg",
                    start=num_args - 1,
...
def _create_named_groups(*groups):
    ...
    for group in groups[0]:
        pattern_string += r'{whitespace}*(?P<{group}>{valid}+),'.format(
            whitespace=_whitespace, group=group, valid=valid)

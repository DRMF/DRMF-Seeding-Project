...
def replace_monics(content):
    ...
    section_pat = re.compile(
        r'section{(.+?)}(.+?)\\subsection\*{Reference',
        re.DOTALL)
...
def rpl_section(match):
    ...
    if args == 0 and params == 0:
        ...
        hyper_pat = re.compile(
            r'{Normalized recurrence relations}(.+?)\\subsection',
            re.DOTALL)
    ...
    hyper_pat = re.compile(
        r'{Normalized recurrence relation}(.+?)\\subsection',
        re.DOTALL)
    hyper_pat.sub(find_name, matchstr)

    monic_pat = re.compile(
        r'p_(?P<open>{)?([^=_;,|$]+?)(?(open)})\(([^=_;,|$]+?)\)(?!\))')
...
def make_monic(match):
    ...
    output = before_m + \
        '{' + match.group(2) + '}@@{' + match.group(3) + '}' + after_m
    return output


def cheby_case_helper(match):

    cheby_pat = re.compile(

...
class Function(object):
    ...
    # create the function object with the given name, abbreviation, regex
    # patterns, and replacement function
    def __init__(
            self,
            macro,
            abbr,
        ...
        if len(regexes) != len(replacements):
            raise ValueError(
                name + " - must provide the same number of replacements as regexes")
...
#        self._name = macro[1:macro.index("{")]
    ...
    def make_subs(self, content):
        ...
        for regex, replacement in zip(self._regexes, self._replace):
            ...
            try:
                ...
                print(
                    "\t\t\t\tERROR WHEN CREATING REGEX - {0} - {1} - {2}".format(regex, self._macro, e))
            ...
            try:
                ...
                print(
                    "ERROR IN MAKING SUBSTITUTIONS FOR FUNCTION {0}: REGEX: {1} REPLACEMENT: {2} - {3}".format(
                        self._macro, regex, replacement, e))
            counter += 1

            # only print out info if verbose is specified
            if self._verbose:
                print(
                    "MAKING SUBS FOR {0} WITH REGEX #{1}".format(
                        self._macro, counter))
                for match in all_matches:
                    print("\t{0}".format(match))

                print(
                    "\tTOTAL SUBS FOR {0} ({1} REGEXES): {2}".format(
                        self._macro, len(
                            self._regexes), total))
    ...
    def get_all_starts(self, content):
        ...
        for regex in self._regexes:
            ...
            for match in pattern.finditer(content):
                ...
                if self._verbose:
                    print("{0} - {1} - {2} - {3}".format(self._name,
                                                         regex, match.start(), match.group()))

                starts.append(match.start())

            # returns a string that maintains the number of lines in the file
            # but will not match further patterns
            def repl(match):
                return "\n" * match.group().count("\n")
    ...
    def count(self, lines):
        ...
        for regex in self._regexes:
            ...
            # replace matches with ' ' to avoid rematching

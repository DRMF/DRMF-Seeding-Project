...
def _replace(match):
    ...
    if (re.search(pattern, match.group(2)) is not None or re.search(
            pattern, match.group(3)) is not None):
        return match.group()

    tempstring = '\\begin{' + \
        match.group(1) + '}\\label{' + match.group(3) + '}%'
...
def _replace2(match):
    ...
    if (re.search(pattern, match.group(2)) is not None):
        return match.group()
...
def replace2(match):
    ...
    if (re.search(pattern,
                  match.group(2)) is not None or re.search(pattern2,
                                                           match.group(2)) is not None or re.search(pattern3,
...
def replace3(match):
    ...
    pattern = re.compile(r'\\nonumber')
    pattern2 = re.compile(r'\\label{')
    if (re.search(pattern, match.group(1)) is not None):
        return match.group()
...
def replace3v2(match):
    ...
    pattern = re.compile(r'\\nonumber')
    pattern2 = re.compile(r'\\label{')
    if (re.search(pattern, match.group(1)) is not None or re.search(
            pattern2, match.group(1)) is not None):
        return match.group()
    tempstring += searchAll(match.group(1))
    tempstring += match.group(1)
    return tempstring
...
def replace4(match):
    ...
    pattern = re.compile(r'\\nonumber')
    pattern2 = re.compile(r'\\label{')
    if (re.search(pattern2, match.group(1)) is not None):
        return match.group()
    tempstring += searchAll(match.group(1))
    tempstring += match.group(1)
    return tempstring
...
def replace5(match):
    ...
    pattern2 = re.compile(r'\\label{')
    if (re.search(pattern, match.group(2)) is not None or re.search(
            pattern2, match.group(2)) is not None):
        return match.group()
    tempstring += searchAll(match.group(2))
    tempstring += match.group(2)
    return tempstring
...
def replacealign(match):
    ...
    labelpattern1 = re.compile(
        r'\\begin{(align)}(.+?)\\label{(.*?)}', re.DOTALL)
    labelpattern2 = re.compile(r'\\\\(.+?)\\label{(.*?)}', re.DOTALL)

    labelpattern2v2 = re.compile(
...
def eqnarray_rpl1(match):
    tempstring = '\\begin{' + \
        match.group(1) + '}\\label{' + match.group(2) + '}%'
...
def eqnarray_replace(match):
    ...
    pattern1 = re.compile(
        r'\\begin{(eqnarray)}\s*\\label{(.*?)}\s*(.*?)(\s*\\end{\1}|\\nonumber)',
        re.DOTALL)
    pattern2 = re.compile(
...
def equation_replace_kk(match):
    ...
    tempstring = '\\begin{' + \
        match.group(1) + '}\\label{' + match.group(2) + r'}%'
    tempstring += searchAll(match.group(3))
...
def LabelAll(content, functions):
    ...
    #{equation} -- Koekoek
    equationpattern = re.compile(
        r'\\begin{(equation)}\s+\\label{(.*?)}\s*(.*?)\\end{\1}',
        re.DOTALL)
    content = equationpattern.sub(equation_replace_kk, content)

    equationpattern = re.compile(
        r'\\begin{(equation)}(?!\\label)\s*(.*?)\s*\\end{\1}', re.DOTALL)

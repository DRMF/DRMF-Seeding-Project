...
def replaceGreekVars(s):
    ...
    for gvar in listOfGVars:
        ...
        # adds a space after the replacement of the greek letter if it's
        # followed by A-Z or a-z
        for i in range(len(newFile)):
            if newFile[i:i + len(newGVar)] == newGVar:
                ...
                elif ord(newFile[i + len(newGVar): i + len(newGVar) + 1]) >= 97 and ord(
                        newFile[i + len(newGVar): i + len(newGVar) + 1]) <= 122:
                    newFile = newFile.replace(newGVar, newGVar + " ")
...
def findArgs(s, functionName):
    ...
    for i in replacings:
        ...
        replacings.extend(re.findall(
            r'' + functionName + '(\[.*\])', i[recurse:]))
        if recurse >= 0:
            # if the function name is found in the replacing, regex is used to find the inner function's arguments
            # regex's string is from the position of the last instance(of
            # function name) plus the length(of function name) till the end
            replacings.extend(re.findall(
                r'' + functionName + '(\[.*\])', i[recurse + len(functionName):]))
    ...
    for i in replacings:
        ...
        for char in i:
            ...
            # if the num open and close are same, the string is appended till
            # that location
...
def ContinuedFractionK(s):
    ...
    for i in arguments:
        ...
        if location < 0:
            location = i.find(", {")
        var = i[1:location]
        s = s.replace("ContinuedFractionK" + i,
                      "\ContinuedFractionK{k}{1}{\infty}@{" + var + "}")
    return s
        ...
        for element in c:
            ...
            if location >= 0:
                ...
                for index in range(element.count("|") + 1):
                    # if it is the first constraints then it is from beginning
                    # to the first location of the |, the loc is increased each
                    # time to get through all of the constraints
                    if index == 0:
                        loc = element.find("|")
    ...
    if (s.find("Element[")) > 0:
        ...
        for index in range(len(constraints)):
            if index == len(constraints) - 1:
                if (var == "Complexes"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \Complex }\n"
                elif (var == "Wholes"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \\NonNegInteger }\n"
                elif (var == "Naturals"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \\NatNumber }\n"
                elif (var == "Integers"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \Integer }\n"
                elif (var == "Irrationals"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \Irrationals }\n"
                elif (var == "Reals"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \Real }\n"
                elif (var == "Rational"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \Rational }\n"
                elif (var == "Primes"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \Prime }\n"
            else:
                if (var == "Complexes"):
                    newFile = newFile + "%      " + \
                        constraints[index] + " \in \Complex &\n"

...
def remove_excess(content):
    ...
    starts = [m.end() for m in start_eq.finditer(content)]
    ends = [m.start() for m in end_eq.finditer(content)]
    print 'starts', starts
    print 'ends', ends
    if len(starts) != len(ends):
        print 'UNEVEN DISTRIBUTION OF BEGIN END EQ'
        sys.exit(-1)
    ...
    for lnum, line in enumerate(lines):
        ...
        # no more ugly % characters after cleaned
